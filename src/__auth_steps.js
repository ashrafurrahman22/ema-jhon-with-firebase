/* 
1. create a firebase project in console.firebase.google.com
2. npm install firebase
3. create firebase.init.js and import getAuth to export auth;
4. firebase setting > authentication > enable email & password
5. create login, signup components setup route
6. attach form field handler and form submit handler
7. npm install --save react-firebase-hooks
8. useCreateUserWithEmailAndPassword from react-firebase-hooks
9. if user is created redirect to the expected page
10. useSignInWithEmailAndPassword for log in
11. create requireAuth components ==> check user exists also trac user location 
12. In route wrap protected components by using require auth components 
*/

/* Hoisting Steps 
1. npm install -g firebase-tools
2. firebase login
3. firebase init
4. firebase deploy
*/